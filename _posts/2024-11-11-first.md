...
- **github username**이 name인 경우, **repository name**을 name.github.io로 설정
- **Public** 체크
...
- 좌측 **Pages** 클릭
- **Source**를 Deploy from a branch로 설정
...
# 에러가 나는 경우
### Ruby 설치 중 cd 디렉토리명 명령어가 작동하지 않을 경우
- **확인 사항**: Clone한 저장 위치가 D드라이브에 있는지 확인합니다. 만약 D드라이브에 있다면 C드라이브로 옮겨주세요.

### 테마 상세 설정 중 커밋이 되지 않을 경우
